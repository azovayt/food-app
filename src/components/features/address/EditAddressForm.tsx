// src/components/features/address/EditAddressForm.tsx
...
const EditAddressForm = (props: Props) => {
  const { initialAddress, loading = false, onSubmit } = props;
  const [form, setForm] = useState<Omit<Address, "addressId" | "createdAt">>({
    addressDescription: "",
    apartmentName: "",
    apartmentNumber: "",
    city: "",
  ...
  useEffect(() => {
    if (initialAddress) {
      const { addressId, createdAt, ...rest } = initialAddress;
      setForm(rest);
    }
  }, [initialAddress]);
  ...
  return (
    <View className="bg-white p-4">
      <View className="flex-row items-center mb-4">
        ...
        <Text className="text-xl font-extrabold text-blue-700 ml-2">
          Adresi Düzenle
        </Text>
      </View>
      ...
      <TouchableOpacity
        className="bg-blue-700 rounded-2xl py-3 mt-6 items-center shadow-md"
